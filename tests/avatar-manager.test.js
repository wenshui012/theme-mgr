...
test('12 avatar grid uses a bounded thumbnail lazy loader rather than main image', async () => { const source = fs.readFileSync(path.join(__dirname, '..', 'src', 'avatar-page.js'), 'utf8'); const f=pageFixture([asset()]); await f.page.mount(); assert.match(f.pageRoot.grid.innerHTML,/placeholder/); assert.match(f.pageRoot.grid.innerHTML,/draggable="false"/); assert.doesNotMatch(f.pageRoot.grid.innerHTML,/main-a/); assert.match(source,/maxConcurrent: 4/); assert.ok(f.stats().observed >= 1); });
